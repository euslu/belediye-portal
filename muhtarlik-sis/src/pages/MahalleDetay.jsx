...
export function loadVisibility(lsKey, sutunlar) {
  ...
  try { const stored = localStorage.getItem(lsKey); if (stored) return { ...defaults, ...JSON.parse(stored) }; } catch (_) {}
  return defaults;
}
export function saveVisibility(lsKey, vis) { localStorage.setItem(lsKey, JSON.stringify(vis)); }
...
function SutunlarDropdown({ sutunlar, visibility, lsKey, onChangeVisibility }) {
  ...
  function toggle(key, checked) {
    ...
    onChangeVisibility(next); saveVisibility(lsKey, next);
  }
  return (
    <div style={{ position: 'relative' }} ref={ref}>
      <button onClick={() => setOpen(v => !v)}
        ...
        <i className="bi bi-layout-three-columns" /> Sütunlar
      </button>
      {open && (
...
function BasvuruEditModal({ row, daireler, onClose, onSaved }) {
  ...
  const [log, setLog] = useState([]); const [logOpen, setLogOpen] = useState(false);
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 600, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #EEEEEE', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#7e7e7e', marginTop: 2 }}>{row.MAHALLE_AD} — {row.BASVURU_TA ? new Date(row.BASVURU_TA).toLocaleDateString('tr-TR') : '—'}</div>
          </div>
        ...
        <div style={{ borderTop: '1px solid #EEEEEE' }}>
          <button onClick={() => { if (!logOpen && !log.length) muhtarlikApi.getBasvuruLog(row.OBJECTID).then(setLog).catch(() => {}); setLogOpen(v => !v); }}
...
function YatirimEditModal({ yatirim, index, ilce, mahalle, onClose, onSaved }) {
  const [form, setForm] = useState({
    aciklama: yatirim.talep || '', tahmini_bedel: yatirim.tahmini_bedel || '',
    daire: yatirim.daire || '', durum: yatirim.durum || '', cevap: yatirim.cevap || '',
  });
  const [saving, setSaving] = useState(false);
  const [error,  setError]  = useState('');
  function setF(k, v) { setForm(p => ({ ...p, [k]: v })); }
  async function handleSave() {
    setSaving(true); setError('');
    try { await muhtarlikApi.updateYatirim(ilce, mahalle, index, form); onSaved(); }
    catch (e) { setError(e.message); } finally { setSaving(false); }
  }
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
...
function BasvurularTab({ ilce, mahalle, daireler }) {
  ...
  const fetch_ = useCallback(() => {
    ...
    muhtarlikApi.getMahalleBasvurular(ilce, mahalle, params).then(setData).catch(() => {}).finally(() => setLoading(false));
  ...
  function renderCell(row, col) {
    const tdStyle = { padding: '10px 14px', verticalAlign: 'top', ...(col.wrap ? { whiteSpace: 'normal', wordBreak: 'break-word' } : { whiteSpace: 'nowrap' }) };
    switch (col.key) {
      case 'BASVURU_TA':     return <td key={col.key} style={{ ...tdStyle, color: '#6b7280' }}>{row.BASVURU_TA ? new Date(row.BASVURU_TA).toLocaleDateString('tr-TR') : '—'}</td>;
      case 'KONUSU':         return <td key={col.key} style={{ ...tdStyle, color: '#212529', maxWidth: 300 }}>{row.KONUSU || '—'}</td>;
      case 'TALEP_GENE':     return <td key={col.key} style={{ ...tdStyle, color: '#6b7280', fontSize: 12, maxWidth: 260 }}>{row.TALEP_GENE || '—'}</td>;
      case 'DAIRE_BASK_ADI': return <td key={col.key} style={{ ...tdStyle }}><span style={{ fontSize: 11, color: '#6b7280' }}>{row.DAIRE_BASK_ADI || '—'}</span></td>;
      case 'CEVAP_SURE':     return <td key={col.key} style={{ ...tdStyle, textAlign: 'center' }}>{row.CEVAP_SURE != null ? <span style={{ fontWeight: 700, color: surRenk(row.CEVAP_SURE) }}>{row.CEVAP_SURE}g</span> : '—'}</td>;
      case 'BIRIM_CEVA':     return <td key={col.key} style={{ ...tdStyle, color: '#374151', fontSize: 12, maxWidth: 240 }}>{row.BIRIM_CEVA || '—'}</td>;
      case 'BIRIM_ISLE':     return <td key={col.key} style={{ ...tdStyle }}>{row.BIRIM_ISLE ? durumBadge(row.BIRIM_ISLE, DURUM_STYLE) : <span style={{ color: '#9ca3af', fontSize: 11 }}>Atanmadı</span>}</td>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 12, color: '#7e7e7e' }}>{loading ? '…' : `${data.toplam.toLocaleString('tr')} başvuru`}</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f9f9f9' }}>
                {visCols.map(col => <th key={col.key} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', borderBottom: '1px solid #EEEEEE', fontSize: 12, whiteSpace: 'nowrap', width: col.width !== 'auto' ? col.width : undefined }}>{col.label}</th>)}
                <th style={{ padding: '10px 14px', borderBottom: '1px solid #EEEEEE', width: 48 }} />
              </tr>
            </thead>
            <tbody>
              {loading ? <tr><td colSpan={visCols.length + 1} style={{ padding: 32, textAlign: 'center', color: '#9ca3af' }}>Yükleniyor…</td></tr>
                : data.rows.length === 0 ? <tr><td colSpan={visCols.length + 1} style={{ padding: 32, textAlign: 'center', color: '#9ca3af' }}>Kayıt bulunamadı</td></tr>
                : data.rows.map((row, i) => (
...
function YatirimlarTab({ ilce, mahalle }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    muhtarlikApi.getMahalleYatirimlar(ilce, mahalle).then(setYatirimlar).catch(() => {}).finally(() => setLoading(false));
  }, [ilce, mahalle]);
  useEffect(() => { load(); }, [load]);
  ...
  function renderCell(y, col) {
    const tdStyle = { padding: '10px 14px', verticalAlign: 'top', ...(col.wrap ? { whiteSpace: 'normal', wordBreak: 'break-word' } : { whiteSpace: 'nowrap' }) };
    switch (col.key) {
      case 'tarih':         return <td key={col.key} style={{ ...tdStyle, color: '#6b7280' }}>{y.tarih || '—'}</td>;
      case 'aciklama':      return <td key={col.key} style={{ ...tdStyle, color: '#212529', maxWidth: 340 }}>{y.talep || '—'}</td>;
      case 'tahmini_bedel': return <td key={col.key} style={{ ...tdStyle, color: '#374151', fontWeight: y.tahmini_bedel ? 500 : 400 }}>{y.tahmini_bedel || '—'}</td>;
      case 'daire':         return <td key={col.key} style={{ ...tdStyle }}><span style={{ fontSize: 11, color: '#6b7280' }}>{y.daire || '—'}</span></td>;
      case 'cevap':         return <td key={col.key} style={{ ...tdStyle, color: '#374151', fontSize: 12, maxWidth: 240 }}>{y.cevap || '—'}</td>;
      case 'durum':         return <td key={col.key} style={{ ...tdStyle }}>{y.durum ? durumBadge(y.durum, YATIRIM_DURUM_STYLE) : <span style={{ color: '#9ca3af', fontSize: 11 }}>—</span>}</td>;
      default: return <td key={col.key} style={tdStyle}>—</td>;
    }
  }
  ...
  return (
    <>
      ...
      {editIdx !== null && <YatirimEditModal yatirim={yatirimlar[editIdx]} index={editIdx} ilce={ilce} mahalle={mahalle} onClose={() => setEditIdx(null)} onSaved={() => { setEditIdx(null); load(); }} />}
    </>
  );
}
...
export default function MahalleDetay() {
  ...
  const [fotoAdSoyad, setFotoAdSoyad] = useState(null);

  useEffect(() => {
    setLoading(true);
    Promise.all([
      muhtarlikApi.getMahalleDetay(ilce, mahalle),
    ...
    fetch(`/api/muhtarbis/muhtar-foto/${enc(ilce)}/${enc(mahalle)}`, {
      headers: { Authorization: `Bearer ${getToken()}`, 'Cache-Control': 'no-cache' },
    })
      .then(r => r.ok ? r.json() : { var: false, url: null })
      .then(d => {
        if (d.var && d.url) { setFotoUrl(d.url); }
        else                { setFotoHata(true); }
        if (d.ad_soyad)     { setFotoAdSoyad(d.ad_soyad); }
        setFotoLoading(false);
      })
      .catch(() => { setFotoHata(true); setFotoLoading(false); });
  }, [ilce, mahalle]);
  ...
  return (
    <div style={{ padding: '32px 40px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(38,175,104,0.12)', color: '#26af68', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🏘️</div>
        <div>
      ...
      {loading ? <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af' }}>Yükleniyor…</div> : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #EEEEEE', padding: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start' }}>
                ...
                <div style={{ flexShrink: 0 }}>
                  {fotoLoading ? (
                    <div style={{ width: 80, height: 80, borderRadius: 12, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <i className="bi bi-person-fill" style={{ fontSize: 32, color: '#cbd5e1' }} />
                    </div>
                  ) : fotoUrl && !fotoHata ? (
                    <img
                      src={fotoUrl}
                      alt="Muhtar"
                      loading="eager"
                      style={{ width: 80, height: 80, borderRadius: 12, objectFit: 'cover', border: '2px solid #e5e7eb', display: 'block', opacity: 0, transition: 'opacity 0.2s' }}
                ...
                {/* Bilgiler */}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 17, fontWeight: 700, color: '#212529', marginBottom: 6 }}>
                    {fotoAdSoyad || detay?.muhtar || mb?.ad?.split('\n')[0] || '—'}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    ...
                    {!fotoLoading && fotoHata && <div style={{ fontSize: 11, color: '#d1d5db', marginTop: 2 }}>Fotoğraf mevcut değil</div>}
                  </div>
                </div>
              </div>
            </div>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #EEEEEE', padding: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[{ label: 'Toplam', value: ist.toplam, color: '#26af68' }, { label: 'Tamamlandı', value: ist.tamamlandi, color: '#22c55e' }, { label: 'Devam Etmekte', value: ist.devam, color: '#3b82f6' }, { label: 'Tamamlanmadı', value: ist.tamamlanmadi, color: '#ef4444' }].map(({ label, value, color }) => (
          ...
          <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: '2px solid #EEEEEE' }}>
            {[{ key: 'basvurular', label: '📋 Başvurular' }, { key: 'yatirimlar', label: '🏗️ Yatırımlar' }].map(t => (
              <button key={t.key} onClick={() => setTab(t.key)} style={{ padding: '10px 24px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: tab === t.key ? 700 : 500, color: tab === t.key ? '#26af68' : '#6b7280', borderBottom: `2px solid ${tab === t.key ? '#26af68' : 'transparent'}`, marginBottom: -2 }}>{t.label}</button>
            ))}
          </div>
